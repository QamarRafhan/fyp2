...
class PasswordResetNotification extends Notification
{
    ...
    {
        $name = $notifiable instanceof ApplicationUser
            ? $notifiable->name
            : trans('application-auth::passwords.mail.generic_recipient');

        return (new MailMessage())
            ->subject(trans('application-auth::passwords.mail.subject', ['name' => config('app.name')]))
            ->greeting(trans('application-auth::passwords.mail.greeting', ['name' => $name]))
            ->line(trans('application-auth::passwords.mail.intro', ['name' => config('app.name')]))
